...
Following the initial custom \gls{crnn} approach, a second \gls{ocr} model was implemented using Microsoft's TrOCR-small-printed architecture. TrOCR employs a Vision Transformer (ViT) encoder to process input image patches and a Transformer decoder to auto-regressively generate the output character sequence. Unlike the \gls{crnn} model which was trained from scratch, TrOCR was fine-tuned from pre-trained weights, leveraging transfer learning from large-scale printed text datasets.

The fine-tuning was performed on cleaned text crops extracted from the \gls{cghd} dataset for 3 epochs. The key implementation details are:
...
\begin{itemize}
    ...
    \item \textbf{Batch processing:} All text crops from a single image are processed in one forward pass
    \item \textbf{Confidence scoring:} Per-token log-probability averaged across the generated sequence
\end{itemize}

The TrOCR model is integrated into the pipeline through a singleton service that lazy-loads the processor and model weights on first invocation, ensuring efficient memory usage. A comparative analysis of \gls{crnn} and TrOCR performance is presented in the Results chapter.

\subsection{Retraining on the 15-Class Merged Taxonomy}

To mitigate inter-class confusion observed in the 61-class experiments, the class taxonomy was consolidated into 15 merged classes. A merge mapping was applied to the original annotations, grouping visually similar subclasses (e.g., \textit{capacitor.polarized} and \textit{capacitor.unpolarized} into \textit{capacitor}). Four \gls{yolo} architectures were then trained on this 15-class dataset for benchmarking:
...
All models were trained on identical data splits with the same augmentation pipeline, confidence threshold of 0.25, and \gls{iou} threshold of 0.45. Comparative results are presented in the Results chapter.
...
This subsection analyzes the computational complexity of the line detection pipeline. Let $H, W$ denote the image dimensions, $n$ the number of nodes (endpoints), $r$ the disk radius for node regions, and $P$ the number of skeleton pixels (typically $P \ll H \times W$).

\textbf{Time Complexity:} Table~\ref{tab:time_complexity} summarizes the time complexity of each operation in the pipeline.
...
\textbf{Space Complexity:} Table~\ref{tab:space_complexity} details the memory requirements of the algorithm.
