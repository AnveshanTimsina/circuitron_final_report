...
\begin{itemize}
    \item \textbf{Backbone:} An extended variant of \gls{elan} serves as the backbone of YOLOv7. It lets the network go deeper without the computational cost ballooning. Hierarchical features get pulled from the input image through this structure, and gradient flow along with feature reuse stay intact throughout.
    
    \item \textbf{Neck:} YOLOv7's neck brings together a \gls{pan} and ELAN-W modules. PAN fuses features across scales, which lifts detection accuracy for objects both small and large. ELAN-W then sharpens the spatial and contextual cues while features are being aggregated.
    
    \item \textbf{Head:} Object classes, bounding box coordinates $(x,y,h,w)$, and objectness scores all come out of the head. Classification and regression are split into separate branches through a decoupled head design, and this split helps convergence. Predictions land at multiple scales so objects of different sizes get picked up.
\end{itemize}
